...
from autoreg_metadata.catalogger.sddi.models import (
    APIService,
    BaseDataset,
    DeviceGroup,
    SDDICategory,
)
from autoreg_metadata.catalogger.sddi.register import SDDICatalogger
from autoreg_metadata.common.models import Coordinate, EndpointMetadata, TimeFrame
...
def sample_endpoint_metadata():
    return EndpointMetadata(
        ...
        timeframe=TimeFrame(
            start_time=datetime(2024, 1, 1), latest_time=datetime(2024, 1, 31)
        ),
        geographical_extent=(
...
def catalogger(mock_ckan):
    return SDDICatalogger(url="https://ckan.example.com", api_key="test-api-key")


def test_init(mock_ckan):
    """Test CKANCatalogger initialization"""
    catalogger = SDDICatalogger(url="https://ckan.example.com", api_key="test-api-key")

    mock_ckan.assert_called_once_with(
        address="https://ckan.example.com", apikey="test-api-key"
    )
...
def test_register_api_service(catalogger, sample_endpoint_metadata):
    ...
    api_service = APIService(
        ...
        url=sample_endpoint_metadata.endpoint_url,
    )
...
def test_register_device_groups(catalogger, sample_endpoint_metadata, sample_data):
    ...
    for idx, (key, _) in enumerate(sample_data.items()):
        ...
        assert device_dict["relationships_as_subject"] == [
            {"subject": key, "object": api_service.name, "type": "links_to"}
        ]
...
def test_error_handling(catalogger, sample_endpoint_metadata):
    ...
    catalogger.ckan_server.call_action = Mock(side_effect=Exception(error_message))
